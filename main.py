...
class ModelWrapper:
    def __init__(self, model, va, vna, vba, vbna):
    ...
    def get_TFIDF(self, text, num):
      ...
      test_tfidf_awesome = self.va.transform(text)
      test_tfidf_not_awesome = self.vna.transform(text)

      # Transforming document-term matrix to a dataframe
      test_tfidf_awesome_df = pd.DataFrame(test_tfidf_awesome.toarray(), columns=self.vba)
      test_tfidf_not_awesome_df = pd.DataFrame(test_tfidf_not_awesome.toarray(), columns=self.vbna)
    ...
    def is_positive(self, df, text1, text2):
    ...
    def image_review_count(self,df):
      ...
      pos1 = self.is_positive(df_new, 'aw_rt', 'naw_rt')
      neg1 = self.is_negative(df_new, 'aw_rt', 'naw_rt')
       
      
      pos2 = self.is_positive(df_new, 'aw_s', 'naw_s')
      neg2 = self.is_negative(df_new, 'aw_s', 'naw_s')
      
      
      return ((pos1 + pos2)+1) / ((neg1 + neg2) + 1)

    def num_verified(self,df):
    ...
    def make_feature_vector(self,iDs, feature_vector, review_data, text):
      ...
      for i in iDs:
          ...
          aw_rt, naw_rt  = self.get_TFIDF(list(current_data['reviewText']), len(current_data['reviewText']))
          aw_s, naw_s  = self.get_TFIDF(list(current_data['summary']), len(current_data['summary']))
          ...
          vote_score = self.get_vote_score(current_data)
          image_score = self.image_review_count(current_data)
          verified = self.num_verified(current_data)
          
          aw_rt1 = current_data['aw_rt'].mean()
          naw_rt1 = current_data['naw_rt'].mean()
          aw_s1 = current_data['aw_s'].mean()
          naw_s1 = current_data['naw_s'].mean()
          time = self.calculate_time_score(current_data)
...
if __name__ == '__main__':
    ...
    model = lr_model
    model_wrapper = ModelWrapper(lr_model, vectorizer_awesome, vectorizer_notawesome, vocabulary_awesome, vocabulary_notawesome)
                                 
    
    # Save the custom object to a pickle file
    with open('amazon_reviews_model.pickle', 'wb') as f:
